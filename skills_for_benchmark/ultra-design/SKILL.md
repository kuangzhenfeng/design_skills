...
核心理念：**界面如同真实的液态玻璃 — 透、亮、有重量、有动感**。动效是必需品而非装饰：加载有转圈、状态有心跳、入场有浮现、滚动有视差、指针有跟随、过渡有流动、交互有灵动反馈。**全部动效通过 Motion 实现，一个都不为"性能"删掉** —— 流畅靠「让动效走合成层（transform/opacity）」与「滚动/指针监听节流」这类温和手段保证，而非牺牲视觉质感；**氛围层零 `filter: blur()`（巨型色斑位图采样、最昂贵），玻璃层只用弱 `backdrop-filter: blur(6px)`（小半径、可控），强 `blur()` 一律禁止**（详见材质章）。
...
3. **视觉质感优先**：氛围色斑可以缓慢流动、hover 阴影可以平滑过渡、所有玻璃元素都享有半透晶体面板（tint + 弱磨砂折射 + 高光 + sheen + 投影）——这些是"看起来高级"的来源，**一律保留**。性能只做温和优化（优先合成层、节流监听、合理 `will-change`，玻璃弱 `backdrop-filter` + 小屏降级，**氛围球零 `filter:blur()`**），不设牺牲视觉的红线。
...
| 要素 | 作用 | 实现 |
|------|------|------|
| **透射（Transmission）** | 背景色彩透过半透 tint + 弱磨砂折射隐约可见——是柔和氛围色而非清晰内容 | 半透 `--glass-tint` + 弱 `backdrop-filter: blur(6px)` |
...
> **为什么玻璃用"弱 `backdrop-filter`"而非强 blur**：强 `backdrop-filter: blur()` 是对玻璃后方整块区域逐像素采样糊化，开销随玻璃面积×模糊半径增长，大面积、大半径（≥20px）会拖垮 GPU。本设计系统改用**弱档 `blur(6px) saturate(1.4)`**——小半径折射，单层开销很小，足以让后方氛围色斑透过玻璃成为柔和色感（而非清晰可见的内容），这正是玻璃"透"的视觉本质；半径一旦加大才是真正的性能黑洞。配合半透 tint + 边缘高光，"透"与"液"两要素都不依赖大半径采样。清晰内容的可读性由 tint 上方的文字/投影对比度保证。
...
## 晶体玻璃材质配方（核心 · 弱 backdrop-filter）

**性能铁律：本设计系统全篇禁止 `filter: blur()`（氛围球那类巨型位图逐像素采样），氛围层只能用单层 radial-gradient mesh。玻璃层允许 `backdrop-filter`，但**统一锁定弱档 `blur(6px) saturate(1.4)`**——小半径折射，单层开销可控；并随小屏 / `prefers-reduced-motion` 降级为纯 tint。**绝对禁止强 `blur()`**（半径 ≥ 20px、大面积铺开），那才是这套语言里真正吃 GPU 的开销。**

玻璃的"透"与"液"由 **半透 tint（alpha 承载通透）+ 弱 `backdrop-filter`（轻度磨砂折射）+ 边缘高光（描边 + inset 白光 + sheen 伪元素）+ 浮起投影（多层 box-shadow）** 叠加实现——真液态质感，开销仅小半径折射 + 常规定位 + 阴影。

**任何晶体元素都包含：半透 tint 透射 + 弱磨砂折射 + 边缘高光 + 镜面投影** 四件套。按钮、输入框、卡片同等享有完整质感——这是高级感的来源，不要为了性能给任何元素降级。

```css
.glass {
  /* 半透 tint 承载"透"——下方氛围色斑透过 tint 隐约可见（氛围感，非清晰内容）。
     弱 backdrop-filter(6px) 做轻度磨砂折射——小半径、开销可控，
     既比纯 tint 更"真玻璃"，又绝不碰强 blur(20px+) 的昂贵大半径采样。 */
...
/* ---- 降级：弱 backdrop-filter 在低端场景退回纯 tint，质感不丢 ---- */
/* 小屏（移动端 backdrop-filter 相对昂贵）与减少动效/透明偏好 → 关掉弱折射，
   玻璃仍有 tint + 边缘高光 + 投影，退回 ultra-design 零 blur 版的纯 tint 玻璃 */
...
> **弱 `backdrop-filter` 用在哪、不用在哪**：
> - **用在**：所有 `.glass` / `.glass-strong` / `.glass-soft` 元素——统一的"轻度磨砂"基线。弱档 `blur(6px)` 单层开销很小，能真折射后方氛围色斑（比纯 tint 更"液态"），叠加边缘高光后质感完整。
> - **不用在**：① `filter: blur()` 氛围球——那是巨型位图逐像素采样，全篇禁止；氛围层只用单层 radial-gradient mesh。② 强 `blur(20px+)`——半径越大开销随面积×半径暴涨，是这套语言里真正吃 GPU 的开销。
>
> **为什么"弱 blur"仍是液态玻璃、且不破坏"透"**：液态感的灵魂是**菲涅尔折射——边缘聚集光线形成发光边、顶面反射环境光形成 sheen**，由 `inset` 白色阴影 + 白色描边 + `::before` sheen 提供，与 blur 半径无关。半透 tint 让氛围色斑透上来保证"透"，弱 backdrop-filter 给一层轻度磨砂、让"透"更柔和真实，浮起投影保证"有重量"。把半径锁在 6px，既拿到真玻璃的折射质感，又避开强 blur 的大半径采样代价。氛围色斑本身已是大半径 `radial-gradient` 软过渡（见下章），玻璃层无需再做二次强模糊。
...
// 玻璃按钮（同样享有弱 backdrop-filter；hover 浮起 + 光泽划过 + active 回弹）
<motion.button
...
- [ ] **氛围层零 `filter: blur()`，玻璃层只有弱 `backdrop-filter: blur(6px) saturate(1.4)`（强 `blur(20px+)` 与 `filter:blur()` 球一律禁止）？小屏 / `prefers-reduced-motion` 下玻璃降级为纯 tint？**
...
| 给氛围色斑加 `filter: blur(80px)`，或给玻璃/遮罩加强 `backdrop-filter: blur(≥20px)` | **氛围球是大半径逐像素采样、最吃 GPU**——氛围改单层 radial-gradient mesh（渐变本就柔和）。**玻璃层允许弱 `backdrop-filter: blur(6px)`**（小半径、开销可控、比纯 tint 更真玻璃，随小屏/`reduced-motion` 降级）；要禁的是强 `blur(20px+)` 与全屏遮罩模糊，那才是面积×半径暴涨的性能黑洞 |
...
| 玻璃放在纯白/纯灰平铺背景 | 必须铺 `.aura` 色斑，玻璃透出色彩才有生命（弱 backdrop-filter 折射的正是这层流动色斑） |
| 只用半透 tint 缺边缘高光 | 必须加白色描边 + inset 白色阴影 + 镜面 sheen，否则只是"塑料片"非"液态玻璃"（质感来自折射高光，与 blur 半径无关） |
