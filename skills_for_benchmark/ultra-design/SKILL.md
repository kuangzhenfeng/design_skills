...
核心理念：**界面如同真实的液态玻璃 — 透、亮、有重量、有动感**。动效是必需品而非装饰：加载有转圈、状态有心跳、入场有浮现、滚动有视差、指针有跟随、过渡有流动、交互有灵动反馈。**全部动效通过 Motion 实现，一个都不为"性能"删掉** —— 流畅靠「让动效走合成层（transform/opacity）」与「滚动/指针监听节流」这类温和手段保证，而非牺牲视觉质感；**氛围层零 `filter: blur()`（巨型色斑位图采样、最昂贵），玻璃层只用弱 `backdrop-filter: blur(4px)` + 静态噪点纹理（磨砂颗粒不靠大 blur），强 `blur()` 一律禁止**（详见材质章）。
...
3. **视觉质感优先**：氛围色斑可以缓慢流动、hover 阴影可以平滑过渡、所有玻璃元素都享有半透晶体面板（tint + 弱磨砂折射 + 高光 + sheen + 投影）——这些是"看起来高级"的来源，**一律保留**。性能只做温和优化（优先合成层、节流监听、合理 `will-change`，玻璃弱 `backdrop-filter: blur(4px)` + 静态噪点纹理 + 小屏降级，**氛围球零 `filter:blur()`**），不设牺牲视觉的红线。
...
| 要素 | 作用 | 实现 |
|------|------|------|
| **透射（Transmission）** | 背景色彩透过半透 tint + 噪点磨砂层隐约可见——是柔和氛围色而非清晰内容 | 半透 `--glass-tint` + 弱 `backdrop-filter: blur(4px) saturate(1.5)` + feTurbulence 噪点 |
...
> **为什么玻璃用「弱 blur + 噪点」而非强 blur**：`backdrop-filter: blur()` 是对玻璃后方整块区域做空间卷积采样，开销随**玻璃面积 × 半径²**增长——面积翻倍线性涨、半径翻倍四倍涨，大半径（≥20px）大面积会拖垮 GPU。本设计系统用**弱档 `blur(4px) saturate(1.5)` + 静态 feTurbulence 噪点纹理**：blur 只给一层极轻折射（半径小、开销可控），磨砂颗粒感**不靠加大 blur 半径**，而是由一张 4% 不透明度的静态噪点贴图提供——人眼识别"磨砂玻璃"时**颗粒感 > 模糊感**，弱 blur 叠噪点看起来像 `blur(16px)` 的质感，开销却只有它零头。噪点是 SVG `feTurbulence` 烘焙成的静态 `background-image`（浏览器只栅格化一次、非每帧采样），便宜且常驻安全。
...
```css
/* 噪点磨砂贴图：feTurbulence 烘焙成静态 background-image，浏览器只栅格化一次、
   非每帧卷积采样——这是"磨砂颗粒感不靠大 blur"的关键。
   freq 0.9 + oct 3 = 细腻致密且有层次的颗粒；亮色用更淡 opacity。
   【铁律·两条】① feComponentTransfer 必须把噪点压到中灰带——原始满量程 [0,1] 噪点的亮像素经
   overlay 会把暗 tint 整片提亮成"电视雪花"；② 强度必须由独立 ::after opacity 控制，
   绝不可混进 background-blend-mode 全强度。 */
:root {
  ...
  --glass-noise-opacity: 0.16;  /* 暗色玻璃：色值压缩后可稍提强度仍不脏；亮色见 [data-theme='light'] */
}

.glass {
  /* 半透 tint 承载"透"——下方氛围色斑透过 tint 隐约可见（氛围感，非清晰内容）。
     弱 backdrop-filter blur(4px) 只给极轻折射（半径小、开销可控）；
     磨砂颗粒感由 ::after 噪点贴图提供，不靠加大 blur 半径。
     【铁律】噪点必须放独立 ::after + opacity，绝不混进 background-blend-mode 全强度——后者无法调强度，
     在玻璃 tint 上会把噪点全强度砸出来，偏亮偏脏像噪点雪花。 */
  position: relative;
...
/* 噪点磨砂层：静态贴图 + overlay 混合，磨砂颗粒感来自这里而非 blur 半径 */
.glass::after, .glass-strong::after, .glass-soft::after {
...
> - **用在**：所有 `.glass` / `.glass-strong` / `.glass-soft` 元素——统一的"轻度磨砂"基线。弱档 `blur(4px)` 单层开销极小，能真折射后方氛围色斑（比纯 tint 更"液态"）；磨砂颗粒感由 `::after` 噪点贴图补足，叠加边缘高光后质感完整。
> - **不用在**：① `filter: blur()` 氛围球——那是巨型位图逐像素采样，全篇禁止；氛围层只用单层 radial-gradient mesh。② 强 `blur(20px+)`——blur 开销随面积×半径²暴涨，半径翻倍四倍涨，是这套语言里真正吃 GPU 的开销。
>
> **为什么"弱 blur + 噪点"仍是液态玻璃、且不破坏"透"**：液态感的灵魂是**菲涅尔折射——边缘聚集光线形成发光边、顶面反射环境光形成 sheen**，由 `inset` 白色阴影 + 白色描边 + `::before` sheen 提供，与 blur 半径无关。半透 tint 让氛围色斑透上来保证"透"，弱 backdrop-filter `blur(4px)` 给一层极轻折射让"透"更柔和真实，**`::after` 噪点贴图补足磨砂颗粒感**（人眼判定"磨砂"，颗粒感重于模糊感），浮起投影保证"有重量"。把半径锁在 4px、磨砂交给静态噪点，既拿到真玻璃的折射质感，又避开强 blur 的大半径采样代价。氛围色斑本身已是大半径 `radial-gradient` 软过渡（见下章），玻璃层无需再做二次强模糊。
...
// 玻璃按钮（同样享有弱 blur + 噪点磨砂层；hover 浮起 + 光泽划过 + active 回弹）
<motion.button
...
| 只用半透 tint + blur、缺噪点磨砂层 | 磨砂颗粒感必须由 `::after` 静态 feTurbulence 噪点贴图补足（overlay + opacity），否则只有"模糊片"没有"磨砂"质感；靠加大 blur 半径补磨砂 = 面积×半径²的性能黑洞 |
| 噪点 URL 漏了 `feComponentTransfer` → 暗色玻璃变"电视雪花" | 原始 feTurbulence 满量程 `[0,1]`，亮像素 ≈0.95 经 overlay 顶亮暗 tint 接近翻倍，整片偏白偏刺。必须 `feComponentTransfer slope='0.35' intercept='0.325'` 压到中灰带 `[0.325,0.675]`，磨砂才干净 |
